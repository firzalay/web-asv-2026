...
MAVLINK_CONNECTION = "COM5"  # <-- GANTI SESUAI PORT DI LEGION GO KAMU
...
CAMERA_SOURCE = 0
CAMERA_WIDTH = (
    480  # frame di-resize ke lebar ini sebelum diproses (biar ringan & cepat)
)
        ...
        try:
            ...
            while True:
                ...
                with ball_hsv_lock:
                    lower = np.array(
                        [
                            ball_hsv_range["h_min"],
                            ball_hsv_range["s_min"],
                            ball_hsv_range["v_min"],
                        ]
                    )
                    upper = np.array(
                        [
                            ball_hsv_range["h_max"],
                            ball_hsv_range["s_max"],
                            ball_hsv_range["v_max"],
                        ]
                    )
                ...
                with ball_lock:
                    ...
                    can_count = (
                        ball_state["present_streak"] >= BALL_STABLE_FRAMES
                        and (now - ball_state["last_count_time"]) >= BALL_COUNT_COOLDOWN
                        and ball_state["count"] < ball_state["total"]
                    )
                    if can_count:
                        ball_state["count"] += 1
...
# State untuk operasi Clear Mission - dikonfirmasi lewat MISSION_ACK dari FC
mission_clear_lock = threading.Lock()
mission_clear_state = {"pending": False}


def request_mission_clear():
    """Hapus SEMUA waypoint yang tersimpan di FC (MISSION_CLEAR_ALL).
    Berguna sebelum mulai rekam rute baru, supaya waypoint lama tidak numpuk
    bareng waypoint baru yang direkam lewat channel 10."""
    conn = master_ref["conn"]
    if conn is None:
        socketio.emit(
    ...
    with mission_clear_lock:
        mission_clear_state["pending"] = True

    print("[Mission] Mengirim MISSION_CLEAR_ALL...")
    conn.mav.mission_clear_all_send(conn.target_system, conn.target_component)
    socketio.emit("mission_clear_status", {"status": "sending"})
...
def request_rc_save_waypoint_setup(channel):
    ...
    if conn is None:
        socketio.emit(
            "rc_setup_result", {"success": False, "message": "Pixhawk belum terhubung."}
        )
        return
    ...
    print(
        f"[RC Setup] Mengirim {param_name} = {SAVE_WAYPOINT_OPTION_VALUE} (Save Waypoint)"
    )
...
# =========================================================================
# RECORD WAYPOINT (opsi kedua selain channel RC) - rekam posisi kapal
# otomatis selama tombol Record aktif, lalu upload hasilnya sebagai mission
# baru ke FC lewat tombol terpisah.
# =========================================================================
import math

# Jarak minimal (meter) antar titik sebelum dianggap waypoint baru.
# Naikkan kalau titik terlalu rapat/numpuk, turunkan kalau rute berkelok tajam.
MIN_RECORD_DISTANCE_M = 3.0

recording_lock = threading.Lock()
recording_state = {"active": False, "points": [], "last_point": None}

mission_upload_lock = threading.Lock()
mission_upload_state = {"uploading": False, "points": [], "sent_count": 0}


def haversine_distance_m(lat1, lon1, lat2, lon2):
    """Hitung jarak antara 2 koordinat GPS (meter) pakai formula haversine."""
...
def request_mission_upload():
    """Upload titik-titik hasil recording sebagai mission baru ke FC.
    Alurnya kebalikan dari download: kita kirim MISSION_COUNT, lalu FC akan
    minta tiap item satu-satu (MISSION_REQUEST_INT), kita balas dengan
    MISSION_ITEM_INT sampai semua terkirim, FC konfirmasi via MISSION_ACK."""
    conn = master_ref["conn"]
    if conn is None:
        socketio.emit(
    ...
    with recording_lock:
        points = list(recording_state["points"])
    ...
    if not points:
        socketio.emit(
            ...
            {
                "success": False,
                "message": "Belum ada waypoint hasil rekam untuk diupload.",
            },
        )
        return

    with mission_upload_lock:
        mission_upload_state["uploading"] = True
        mission_upload_state["points"] = points
        mission_upload_state["sent_count"] = 0

    print(f"[Mission Upload] Mengirim MISSION_COUNT = {len(points)}")
    conn.mav.mission_count_send(
        conn.target_system, conn.target_component, len(points), 0
    )
    socketio.emit("mission_upload_status", {"status": "sending", "total": len(points)})
...
def mavlink_listener():
    ...
    while True:
        try:
            ...
            print(
                f"[MAVLink] Heartbeat diterima dari system {master.target_system}, "
            ...
            while True:
                ...
                if msg_type == "GLOBAL_POSITION_INT":
                    ...
                    latest_data["heading"] = (
                        msg.hdg / 100.0 if msg.hdg != 65535 else latest_data["heading"]
                    )

                    # --- Record Waypoint: catat titik baru kalau sudah cukup jauh dari titik terakhir ---
                    with recording_lock:
                        if recording_state["active"]:
                            lat, lon = latest_data["lat"], latest_data["lon"]
                            if lat != 0 or lon != 0:
                                last = recording_state["last_point"]
                                far_enough = (
                                    last is None
                                ...
                                if far_enough:
                                    point = {
                                        "seq": len(recording_state["points"]),
                                        "lat": lat,
                                        "lon": lon,
                                        "alt": 0,
                                        "timestamp": time.time(),
                                    }
                                    recording_state["points"].append(point)
                                    recording_state["last_point"] = point
                                    socketio.emit("recording_point_added", point)

                elif msg_type == "HEARTBEAT":
                    latest_data["armed"] = bool(
                        msg.base_mode & mavutil.mavlink.MAV_MODE_FLAG_SAFETY_ARMED
                    )
                    new_mode = ROVER_MODE_MAP.get(
                    ...
                    if new_mode == "AUTO" and mode_tracker["previous_mode"] != "AUTO":
                        print(
                            "[Mode] AUTO baru saja diaktifkan -> auto-refresh tampilan mission untuk verifikasi"
                        )
                    ...
                    latest_data["battery_voltage"] = round(
                    ...
                    if text:
                        ...
                        if entry["is_waypoint_event"]:
                            print(
                                f"[WP Save Event] {text} @ ({entry['lat']:.6f}, {entry['lon']:.6f})"
                            )
                    ...
                    if pending_param:
                        ...
                        if received_name == pending_param:
                            ...
                            print(
                                f"[RC Setup] {pending_param} dikonfirmasi FC = {msg.param_value} "
                    ...
                    with mission_clear_lock:
                        was_clearing = mission_clear_state["pending"]
                        mission_clear_state["pending"] = False
                    with mission_upload_lock:
                        was_uploading = mission_upload_state["uploading"]
                        mission_upload_state["uploading"] = False

                    if was_clearing:
                        success = msg.type == mavutil.mavlink.MAV_MISSION_ACCEPTED
                        print(
                            f"[Mission] Clear mission {'berhasil' if success else 'gagal'} "
                            f"(ack type: {msg.type})"
                        )
                        if success:
                            with mission_lock:
                        ...
                        success = msg.type == mavutil.mavlink.MAV_MISSION_ACCEPTED
                        print(
                            f"[Mission Upload] {'Berhasil' if success else 'Gagal'} "
                            f"(ack type: {msg.type})"
                        )
                        socketio.emit(
                            "mission_upload_result",
                        ...
                        if success:
                            # Refresh tampilan mission dari FC untuk verifikasi hasil upload
                            request_mission_download()

                elif msg_type in ("MISSION_REQUEST_INT", "MISSION_REQUEST"):
                    # FC sedang minta item mission satu-per-satu selama proses upload
                    with mission_upload_lock:
                        uploading = mission_upload_state["uploading"]
                        points = mission_upload_state["points"]
                    ...
                    if uploading:
                        ...
                        if seq < len(points):
                            ...
                            master.mav.mission_item_int_send(
                                ...
                                0,
                                0,
                                0,  # param1-4 (tidak dipakai untuk waypoint biasa)
                                int(pt["lat"] * 1e7),
                                int(pt["lon"] * 1e7),
                                float(pt.get("alt", 0)),
                            ...
                            with mission_upload_lock:
                                mission_upload_state["sent_count"] = seq + 1
                            socketio.emit(
                                "mission_upload_progress",
                                {"sent": seq + 1, "total": len(points)},
                            )
                        else:
                            print(
                                f"[Mission Upload] FC minta seq {seq} di luar jangkauan "
                                f"({len(points)} titik direkam)"
                            )
                    ...
                    with mission_lock:
                        item = {
                            ...
                            "alt": msg.z,
                            "command": msg.command,
                            "command_name": MAV_CMD_NAMES.get(
                                msg.command, f"CMD_{msg.command}"
                            ),
                        }
                    ...
                    if next_seq < total:
                        master.mav.mission_request_int_send(
                            master.target_system, master.target_component, next_seq
                        )
                    else:
...
def handle_clear_mission():
    print("[Web] Clear mission diminta dari dashboard")
    request_mission_clear()
...
def handle_start_recording():
    with recording_lock:
        ...
        recording_state["points"] = []
        recording_state["last_point"] = None
    print("[Recording] Mulai rekam waypoint")
    socketio.emit("recording_status", {"active": True, "count": 0})
...
def handle_stop_recording():
    with recording_lock:
        ...
        count = len(recording_state["points"])
    print(f"[Recording] Berhenti rekam, total {count} titik terekam")
    socketio.emit("recording_status", {"active": False, "count": count})


@socketio.on("clear_recorded_points")
def handle_clear_recorded_points():
    with recording_lock:
        recording_state["points"] = []
        recording_state["last_point"] = None
        active = recording_state["active"]
    print("[Recording] Hasil rekaman dihapus")
    socketio.emit("recording_status", {"active": active, "count": 0})
    socketio.emit("recording_cleared", {})


@socketio.on("upload_recorded_mission")
def handle_upload_recorded_mission():
    print("[Web] Upload mission hasil rekaman diminta dari dashboard")
    request_mission_upload()
